...
const AddNewPaletteModal = ({ navigation }) => {
  ...
  const onSelectColor = useCallback(
    (color, selected) => {
      if (selected) {
        const newColors = { ...paletteColors, [color.hexCode]: color };
        console.log('newColors--> ', newColors);
        setPaletteColors(newColors);
      } else {
        const rmColors = { ...paletteColors };
        delete rmColors[color.hexCode];
        console.log('rmColors--> ', rmColors);
        setPaletteColors(rmColors);
      }
    },
    [paletteColors],
  );
  ...
  const onSubmit = () => {
    const minCount = 5;
    const colors = Object.keys(paletteColors).map((key) => paletteColors[key]);
    console.log('COLORS: ', colors);
    console.log('paletteName: ', paletteName);
    if (!paletteName || colors.length < minCount) {
      var message;
      if (!paletteName && colors.length < minCount) {
        message = `Please enter a palette name and select at least ${minCount} colors`;
      } else if (!paletteName) {
        message = 'Please enter a palette name';
      } else {
        message = `Please  select at least ${minCount} colors`;
      }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.listContainer}>
        <FlatList
          ...
          renderItem={({ item }) => (
            <ColorItem color={item} onSelect={onSelectColor} />
          )}
        />
      </View>
...
const ColorItem = ({ color, onSelect = () => {} }) => {
  const [selected, setSelected] = useState(false);

  const toggleSwitch = useCallback(() => {
    const newState = !selected;
    setSelected(newState);
    onSelect(color, newState);
  }, [selected, onSelect, color]);

  return (
    <TouchableHighlight
      underlayColor="#DDDDDD"
      onPress={() => toggleSwitch(selected)}>
      <View style={styles.item}>
        <Text>{color.colorName}</Text>
        <Switch onValueChange={toggleSwitch} value={selected} />
      </View>
    </TouchableHighlight>
  );
};
